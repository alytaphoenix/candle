...
impl QMetalStorage {
    ...
    fn fwd_mv(
        ...
        // known Metal-kernel quirk elsewhere in this same file, see its own
        // "Fixing a bug in Metal for GGML" comment in `candle-metal-
        // kernels`' tuning table), `Q5_1` an entirely-zeroed row 0 at
        // `batch == 4` with rows 1-3 correct (a distinct failure shape,
        // root cause not chased further). Root cause not chased for
        // either, since no target model here needs them through this
        // dispatch path -- this list is deliberately built from "verified
        // correct by a real differential test," not "looks safe by
        // reading the kernel source," after these two proved that
        // shortcut wrong twice. `Q8_1` is also absent: its own mat-vec
        // Metal kernel (`kernel_mul_mv_q8_1_f32`) doesn't exist anywhere
        // in this fork's vendored `.metal` source at all -- a pre-existing
    ...
    pub fn fwd(
        ...
        // amortized at small batch, the same mechanism already found and
        // fixed once in this fork's MoE `indexed_moe_forward` dispatch
        // (`use_mv`/`mv_id` vs. `mm_id`). `8` matches that fix's own
        // threshold, chosen the same way (a conservative margin below an
        ...
        // path for a dtype this fix doesn't need to touch.
        //
    ...
    pub fn indexed_moe_forward(
        ...
        if use_mv {
            ...
            // the device's fixed budget regardless of tuning (Metal MoE
            // Phase 3, see ratatoskr/DESIGN.md section 15 -- found live via
            // a real ~2594-token prompt).
