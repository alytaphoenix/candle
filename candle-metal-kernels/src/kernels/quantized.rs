...
pub fn call_mm_id_expert_counts<E: EncoderProvider>(
    ...
    encoder.dispatch_thread_groups(
        MTLSize { width: n_expert.max(1) as usize, height: 1, depth: 1 },
    ...
    set_params!(encoder, ((ids, ids_offset), (&mut *counts, 0), nei0, nei1, nbi1, n_expert));
    encoder.dispatch_thread_groups(
...
pub fn call_quantized_matmul_mm_id(
    ...
    let name = match dtype {
        ...
        GgmlDType::Q8_1 => Err(MetalKernelError::UnsupportedDTypeForOp("Q8_1", "qmatmul_id"))?,
    ...
    let encoder: &ComputeCommandEncoder = encoder.as_ref();
    encoder.set_compute_pipeline_state(&pipeline);
    debug_group!(encoder, "qmm_mm_id {name} n_tokens={nei1} K={ne00} N={ne01} n_expert={ne02}");
...
pub fn mm_id_max_nei1(device: &Device, nei0: i64) -> i64 {
    ...
    // this stack has measured (16384/32768/65536), but not guaranteed by
    // the API. Floor `max_smem` to a multiple of 16 first so this stays an
...
/// Chunked wrapper over `call_quantized_matmul_mm_id` (Metal MoE Phase 3, see
/// ratatoskr/DESIGN.md section 15): splits a batch whose `nei1` (n_tokens)
/// exceeds `max_nei1` into sequential dispatches of at most `max_nei1`
...
pub fn call_quantized_matmul_mv_id(
    ...
    let encoder: &ComputeCommandEncoder = encoder.as_ref();
    encoder.set_compute_pipeline_state(&pipeline);
    debug_group!(encoder, "qmm_mv_id {name} n_tokens={nei1} K={ne00} N={ne01} n_expert={ne02}");
...
fn mv_id_dispatch_params(dtype: GgmlDType) -> Result<(usize, usize, usize, &'static str), MetalKernelError> {
    Ok(match dtype {
        ...
        GgmlDType::BF16 => return Err(MetalKernelError::UnsupportedDTypeForOp("BF16", "qmatmul_mv_id")),
        GgmlDType::Q8_1 => return Err(MetalKernelError::UnsupportedDTypeForOp("Q8_1", "qmatmul_mv_id")),
        GgmlDType::Q8K => return Err(MetalKernelError::UnsupportedDTypeForOp("Q8K", "qmatmul_mv_id")),
    })
}
