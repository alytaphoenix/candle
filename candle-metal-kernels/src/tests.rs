...
// for MoE expert dispatch -- see ratatoskr/DESIGN.md section 15 in the
// yggdrasil monorepo for the full context). Confirms the kernel this whole
// effort depends on actually loads as a Metal compute pipeline from the
// compiled metallib, since it has `[[host_name]]` instantiations but (at
// the time this test was added) zero Rust callers anywhere in this crate
// or candle-core -- named kernels aren't dead-code-eliminated from the
...
// De-risk spike for `call_quantized_matmul_mv_id` (Metal MoE Phase 2, see
// ratatoskr/DESIGN.md section 15): a differential test against
// `call_quantized_matmul_mm_id`, not a hand-computed reference. At
// n_tokens=1 both kernels implement the identical ggml op, and mm_id is
// already validated (the tests above, plus live end-to-end validation
// against a real model in Phase 1) -- so mm_id's own output is a complete,
// free correctness oracle for mv_id's binding order, dispatch shape, and
// the `ne1 = nei0` field (the exact bug class review caught twice in
// mm_id: candle's own upstream PR #3555, and this project's first mm_id
// attempt). Same F32 dtype and shape as
...
fn kernel_mul_mv_id_f32_matches_mm_id() {
    ...
    let mm_dst_buf = device.new_buffer(out_elems * std::mem::size_of::<f32>(), RESOURCE_OPTIONS).unwrap();
    let mv_dst_buf = device.new_buffer(out_elems * std::mem::size_of::<f32>(), RESOURCE_OPTIONS).unwrap();
...
fn kernel_mul_mv_id_f32_broadcast_matches_mm_id() {
    ...
    // selected experts.
    let src1: Vec<f32> = (0..n_tokens * n_in).map(|idx| (idx as f32) * 0.001).collect();
...
// `n_tokens == 1` (Metal MoE MTP-verify-step investigation, see
// ratatoskr/DESIGN.md's native-MTP section): every existing mv_id test above
...
// the entire native-MTP verify-window range this investigation cares about.
...
fn kernel_mul_mv_id_at_batch_n_matches_sequential_single_token_calls_bit_exact() {
    ...
    for n_tokens in [2usize, 3, 5, 8] {
        ...
        let out_elems = n_tokens * n_expert_used * n_out;
        let batched_dst_buf =
            device.new_buffer(out_elems * std::mem::size_of::<f32>(), RESOURCE_OPTIONS).unwrap();
        ...
        for t in 0..n_tokens {
            let single_dst_buf = device
                .new_buffer(n_expert_used * n_out * std::mem::size_of::<f32>(), RESOURCE_OPTIONS)
                .unwrap();
            ...
            let batched_slice = &batched_got[t * n_expert_used * n_out..(t + 1) * n_expert_used * n_out];
            assert_eq!(
                batched_slice, single_got.as_slice(),
...
// representative points in the native-MTP verify-window range (2, the
...
// Phase 1 timing table (Metal MoE MTP-verify-step investigation, see
// ratatoskr/DESIGN.md's native-MTP section): mv_id vs. mm_id wall-clock at
// batch sizes spanning decode (1) through the native-MTP verify window
...
// De-risk spike for `call_quantized_matmul_mm_id_chunked` (Metal MoE Phase 3,
// see ratatoskr/DESIGN.md section 15): unlike the mv_id spike above, this
// compares the chunked wrapper against the *same* kernel
...
fn run_mm_id_chunked_case(
    ...
    let ids_buf = new_buffer(&device, &ids);
    let out_elems = n_tokens * n_expert_used * n_out;
    let unchunked_dst = device.new_buffer(out_elems * std::mem::size_of::<f32>(), RESOURCE_OPTIONS).unwrap();
    let chunked_dst = device.new_buffer(out_elems * std::mem::size_of::<f32>(), RESOURCE_OPTIONS).unwrap();
...
// Proves the ceiling this phase exists to remove is actually gone: at
// nei0=8 (this stack's real top-k) and a token count derived from the real
...
fn kernel_mul_mm_id_chunked_succeeds_above_the_real_device_ceiling() {
    ...
    let n_out = 64usize;
    let n_in = 32usize;
    let nei0 = 8usize; // this stack's real top-k

    let max_nei1 = mm_id_max_nei1(&device, nei0 as i64);
    assert!(max_nei1 > 0, "test assumption: this device can fit at least one chunk");
    ...
    let src1: Vec<f32> = (0..n_tokens * nei0 * n_in)
        ...
        .collect();
    let ids: Vec<i32> = (0..n_tokens * nei0).map(|idx| ((idx * 7 + 3) % n_expert) as i32).collect();

    let src0_buf = new_buffer(&device, &src0);
    let src1_buf = new_buffer(&device, &src1);
    let ids_buf = new_buffer(&device, &ids);
    let out_elems = n_tokens * nei0 * n_out;
    let dst_buf = device.new_buffer(out_elems * std::mem::size_of::<f32>(), RESOURCE_OPTIONS).unwrap();
...
fn gdn_scan_build_solve_reference(k_c: &[f32], log_g_c: &[f32], beta_c: &[f32], chunk: usize, hk: usize) -> Vec<f32> {
...
fn run_gdn_scan_build_solve_and_check(bhnc: usize, hk: usize) {
    ...
    assert!(max_diff < 1e-3, "bhnc={bhnc} hk={hk}: build+solve mismatch, max diff = {max_diff}");
}
...
fn kernel_gdn_chunked_scan_build_and_solve_respects_nonzero_buffer_offsets() {
    ...
    let k_off = BufferOffset { buffer: &k_buf, offset_in_bytes: decoy_k.len() * f32_size };

    let scalar_packed: Vec<f32> = [&decoy_scalar[..], &log_g_c[..], &beta_c[..]].concat();
    let scalar_buf = new_buffer(&device, &scalar_packed);
    let log_g_off = BufferOffset { buffer: &scalar_buf, offset_in_bytes: decoy_scalar.len() * f32_size };
    let beta_off =
        BufferOffset { buffer: &scalar_buf, offset_in_bytes: (decoy_scalar.len() + log_g_c.len()) * f32_size };

    let attn_buf = device
        .new_buffer(bhnc * chunk * chunk * f32_size, RESOURCE_OPTIONS)
        .unwrap();

    call_gdn_chunked_scan_build_and_solve_f32(&device, &encoder, &kernels, bhnc, hk, &k_off, &log_g_off, &beta_off, &attn_buf)
        .unwrap();
    drop(encoder);
    commands.wait_until_completed().unwrap();
    ...
    assert!(max_diff < 1e-3, "nonzero-offset build+solve mismatch, max diff = {max_diff}");
}
